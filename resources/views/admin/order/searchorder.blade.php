        ...
        <div class="card rounded shadow border-0">
          <div class="card-body p-5 bg-white rounded">
            ...
            <div class="table-responsive">
                <ul class="pagination">
                    @if($page >= 2)
                    <li class="page-item"><a class="page-link" href="/admin/searchorderlist/{{ $page-1 }}?email={{$email}}&productname={{$productname}}&shippingaddress={{$shippingaddress}}">Previous</a></li>
                    @endif
                    @for($i = 1; $i <= $number_of_page; $i++)
                    @if($page == $i)
                    <li class="page-item active"><a class="page-link" href="/admin/searchorderlist/{{ $i }}?email={{$email}}&productname={{$productname}}&shippingaddress={{$shippingaddress}}">{{$i}}</a></li>
                    @continue
                    @endif
                    <li class="page-item"><a class="page-link" href="/admin/searchorderlist/{{ $i }}?email={{$email}}&productname={{$productname}}&shippingaddress={{$shippingaddress}}">{{$i}}</a></li>
                    @endfor
                    @if($page < $number_of_page)
                    <li class="page-item"><a class="page-link" href="/admin/searchorderlist/{{ $page+1 }}?email={{$email}}&productname={{$productname}}&shippingaddress={{$shippingaddress}}">Next</a></li>
                    @endif
                </ul>
            </div>
          </div>
        ...
        <div class="modal-body">
            <form action="/admin/searchorderlist" method="GET">
                <div class="form-group">
                  <label>Email</label>
                  <input type="text" class="form-control" placeholder="Enter Email ID" name="email" value="{{ $email }}">
                </div>
                <div class="form-group">
                  <label>Product Name:</label>
                  <input type="text" class="form-control" placeholder="Enter Name" name="productname" value="{{ $productname }}">
                </div>
                <div class="form-group">
                  <label>Shipping Address:</label>
                  <input type="text" class="form-control" placeholder="Enter Phone Number" name="shippingaddress" value="{{ $shippingaddress }}">
                </div>
        </div>

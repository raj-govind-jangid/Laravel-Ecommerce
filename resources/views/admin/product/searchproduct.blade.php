        ...
        <div class="card rounded shadow border-0">
          <div class="card-body p-5 bg-white rounded">
            ...
            <div class="table-responsive">
                <ul class="pagination">
                    @if($page >= 2)
                    <li class="page-item"><a class="page-link" href="/admin/searchproduct/{{ $page-1 }}?brandname={{$brandname}}&name={{$name}}&status={{$status}}">Previous</a></li>
                    @endif
                    @for($i = 1; $i <= $number_of_page; $i++)
                    @if($page == $i)
                    <li class="page-item active"><a class="page-link" href="/admin/searchproduct/{{ $i }}?brandname={{$brandname}}&name={{$name}}&status={{$status}}">{{$i}}</a></li>
                    @continue
                    @endif
                    <li class="page-item"><a class="page-link" href="/admin/searchproduct/{{ $i }}?brandname={{$brandname}}&name={{$name}}&status={{$status}}">{{$i}}</a></li>
                    @endfor
                    @if($page < $number_of_page)
                    <li class="page-item"><a class="page-link" href="/admin/searchproduct/{{ $page+1 }}?brandname={{$brandname}}&name={{$name}}&status={{$status}}">Next</a></li>
                    @endif
                </ul>
            </div>
          </div>
